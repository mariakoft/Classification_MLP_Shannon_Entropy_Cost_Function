...
# load dataset
dataframe1 = pandas.read_csv("iris.csv", header=None)
dataset = dataframe1.values
X = dataset[:,0:4].astype(float)
#standardized_X = preprocessing.scale(X)  #to standarize data
Y = dataset[:,4]
...
model = Sequential()
model.add(Dense(8, input_dim=X.shape[1], activation='softmax'))
...
history = model.fit(X, dummy_y, validation_split=0.15, epochs=800, batch_size=len(X),
                     shuffle = True)
